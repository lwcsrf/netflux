...
ApplyDiffPatch = AgentFunction(
    ...
    desc=("Apply unified diff patches (git-style) to one or more text files. "
          ...
          "* Applies all hunks found in the document as an atomic transaction (rollback on failure).\n"
          ...
          "* On failure, raises a concise exception with "
          "actionable details and sample of failing hunk excerpt.\n"
    ...
    args=[
        FunctionArg(
            "diff_content",
            str,
            "Path to file containing the patch or markdown (changes document), or the dereferenced content itself. "
...
"""
    ...
    user_prompt_template=(
        "Apply the following `diff_content`\n\n{diff_content}"
    ),
